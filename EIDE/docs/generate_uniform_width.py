#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
统一宽度嵌入式软件架构图生成脚本
确保每层外围边框宽度一致，左右对齐
"""

import os
import subprocess

os.makedirs('docs/assets', exist_ok=True)

GRAPHVIZ_PATH = r"D:\Soft_Install_HL\graphviz\bin"
DOT_EXE = os.path.join(GRAPHVIZ_PATH, "dot.exe")

# 统一宽度配置：所有层有相同的"视觉节点数"
# 驱动层有6个节点，所以设为基准
BASE_NODES = 6

# 各层节点数
LAYER_NODES = {
    'app': 3,      # 应用层
    'mw': 2,       # 中间件
    'drv': 6,      # 驱动层（基准）
    'hal': 2,      # HAL
    'hw': 4,       # 硬件层
    'peri': 5      # 外设层
}

# 计算每层需要的填充节点数
def get_fillers(node_count):
    """返回左右填充节点数，使总节点数达到BASE_NODES"""
    needed = BASE_NODES - node_count
    if needed <= 0:
        return (0, 0)
    # 尽量均匀分布在左右
    left = needed // 2
    right = needed - left
    return (left, right)

# 生成填充节点定义
def generate_fillers(prefix, count):
    """生成指定数量的不可见填充节点"""
    if count <= 0:
        return ""
    
    fillers = []
    for i in range(count):
        fillers.append(f'{prefix}_fill{i} [label="", style="invisible", width=1.5, height=0.01];')
    return "\n        " + "\n        ".join(fillers)

# 生成填充节点的引用（用于rank=same）
def generate_filler_refs(prefix, count):
    """生成填充节点的引用列表"""
    if count <= 0:
        return ""
    
    refs = []
    for i in range(count):
        refs.append(f'{prefix}_fill{i}')
    return " ".join(refs)

# 统一宽度 DOT 定义
uniform_dot = '''digraph UniformArchitecture {
    // 统一宽度配置
    graph [
        rankdir=TB
        splines=ortho
        nodesep=0.2
        ranksep=0.4
        fontname="Arial, sans-serif"
        fontsize=13
        label="F413 嵌入式软件架构"
        labelloc=t
        pad=0.4
        bgcolor="#FFFFFF"
        newrank=true
    ];
    
    // 节点样式
    node [
        fontname="Arial, sans-serif"
        fontsize=10
        shape=rect
        style="rounded,filled"
        penwidth=1.0
        margin=0.1
        width=1.8
        height=0.6
        fillcolor="#FFFFFF"
        color="#333333"
    ];
    
    // 连线样式
    edge [
        fontname="Arial, sans-serif"
        fontsize=8
        arrowsize=0.6
        penwidth=1.0
        color="#555555"
        arrowhead=normal
    ];

    // ===== 应用层 (3个节点，需要3个填充) =====
    subgraph cluster_app {
        label="应用层 (Application)"
        style="filled,rounded"
        fillcolor="#E8F4FF"
        color="#1976D2"
        penwidth=1.5
        
        // 左边1个填充 + 3个实际节点 + 右边2个填充 = 总共6个
        app_fillL0 [label="", style="invisible", width=1.5, height=0.01];
        
        app_parser [label="userparser.c\\n系统初始化与任务创建", fillcolor="#FFFFFF", color="#1976D2"];
        app_screen [label="screen.c\\nUI显示与按键处理", fillcolor="#FFFFFF", color="#1976D2"];
        app_task [label="app_task.c\\n定时任务调度框架", fillcolor="#FFFFFF", color="#1976D2"];
        
        app_fillR0 [label="", style="invisible", width=1.5, height=0.01];
        app_fillR1 [label="", style="invisible", width=1.5, height=0.01];
    }

    // ===== 中间件层 (2个节点，需要4个填充) =====
    subgraph cluster_mw {
        label="中间件 (Middleware)"
        style="filled,rounded"
        fillcolor="#F0F9E8"
        color="#388E3C"
        penwidth=1.5
        
        // 左边2个填充 + 2个实际节点 + 右边2个填充 = 总共6个
        mw_fillL0 [label="", style="invisible", width=1.5, height=0.01];
        mw_fillL1 [label="", style="invisible", width=1.5, height=0.01];
        
        mw_freertos [label="FreeRTOS\\n任务管理与调度", fillcolor="#FFFFFF", color="#388E3C"];
        mw_apptask [label="AppTask框架\\n定时任务链表", fillcolor="#FFFFFF", color="#388E3C"];
        
        mw_fillR0 [label="", style="invisible", width=1.5, height=0.01];
        mw_fillR1 [label="", style="invisible", width=1.5, height=0.01];
    }

    // ===== 驱动层 (6个节点，无需填充) =====
    subgraph cluster_drv {
        label="驱动层 (Driver)"
        style="filled,rounded"
        fillcolor="#FFF3E0"
        color="#F57C00"
        penwidth=1.5
        
        drv_uart [label="UART1-7\\n串口通信 (DMA)", fillcolor="#FFFFFF", color="#F57C00"];
        drv_gpio [label="GPIO\\n输入输出控制", fillcolor="#FFFFFF", color="#F57C00"];
        drv_tim [label="TIM7/10/14\\n定时器驱动", fillcolor="#FFFFFF", color="#F57C00"];
        drv_adc [label="ADC\\n模拟信号采集", fillcolor="#FFFFFF", color="#F57C00"];
        drv_i2c [label="Software I2C\\nEEPROM存储", fillcolor="#FFFFFF", color="#F57C00"];
        drv_1wire [label="1-Wire\\nRFID/DS2401", fillcolor="#FFFFFF", color="#F57C00"];
    }

    // ===== 硬件抽象层 (2个节点，需要4个填充) =====
    subgraph cluster_hal {
        label="硬件抽象层 (HAL)"
        style="filled,rounded"
        fillcolor="#FCE4EC"
        color="#C2185B"
        penwidth=1.5
        
        // 左边2个填充 + 2个实际节点 + 右边2个填充 = 总共6个
        hal_fillL0 [label="", style="invisible", width=1.5, height=0.01];
        hal_fillL1 [label="", style="invisible", width=1.5, height=0.01];
        
        hal_board [label="board.h/c\\n引脚与外设配置", fillcolor="#FFFFFF", color="#C2185B"];
        hal_stm32 [label="STM32F4xx HAL\\n外设驱动库", fillcolor="#FFFFFF", color="#C2185B"];
        
        hal_fillR0 [label="", style="invisible", width=1.5, height=0.01];
        hal_fillR1 [label="", style="invisible", width=1.5, height=0.01];
    }

    // ===== 硬件层 (4个节点，需要2个填充) =====
    subgraph cluster_hw {
        label="硬件层 (Hardware)"
        style="filled,rounded"
        fillcolor="#E8EAF6"
        color="#303F9F"
        penwidth=1.5
        
        // 左边1个填充 + 4个实际节点 + 右边1个填充 = 总共6个
        hw_fillL0 [label="", style="invisible", width=1.5, height=0.01];
        
        hw_mcu [label="STM32F4xx\\nCortex-M4 100MHz", fillcolor="#FFFFFF", color="#303F9F"];
        hw_uart [label="UART\\n7路串口", fillcolor="#FFFFFF", color="#303F9F"];
        hw_gpio [label="GPIO\\n多路输入输出", fillcolor="#FFFFFF", color="#303F9F"];
        hw_eeprom [label="AT24C02\\n2Kb EEPROM", fillcolor="#FFFFFF", color="#303F9F"];
        
        hw_fillR0 [label="", style="invisible", width=1.5, height=0.01];
    }

    // ===== 外部设备层 (5个节点，需要1个填充) =====
    subgraph cluster_peri {
        label="外部设备 (Peripherals)"
        style="filled,rounded"
        fillcolor="#FFF8E1"
        color="#FF8F00"
        penwidth=1.5
        
        // 左边0个填充 + 5个实际节点 + 右边1个填充 = 总共6个
        per_motor [label="无刷电机\\nA/B双通道", fillcolor="#FFFFFF", color="#FF8F00"];
        per_lcd [label="LCD显示屏\\n4.3寸触摸", fillcolor="#FFFFFF", color="#FF8F00"];
        per_pedal [label="脚踏板\\n油门+按键", fillcolor="#FFFFFF", color="#FF8F00"];
        per_hand [label="手柄\\nRFID识别", fillcolor="#FFFFFF", color="#FF8F00"];
        per_pump [label="灌注/注水泵\\n流量控制", fillcolor="#FFFFFF", color="#FF8F00"];
        
        peri_fillR0 [label="", style="invisible", width=1.5, height=0.01];
    }

    // ===== 连接关系 =====
    app_parser -> mw_freertos [color="#1976D2"];
    app_screen -> mw_apptask [color="#1976D2"];
    app_task -> mw_freertos [color="#1976D2"];
    app_task -> mw_apptask [color="#1976D2"];

    mw_freertos -> drv_uart [color="#388E3C"];
    mw_freertos -> drv_gpio [color="#388E3C"];
    mw_freertos -> drv_tim [color="#388E3C"];
    mw_apptask -> drv_uart [color="#388E3C", style=dashed];
    mw_apptask -> drv_gpio [color="#388E3C", style=dashed];

    drv_uart -> hal_board [color="#F57C00"];
    drv_gpio -> hal_board [color="#F57C00"];
    drv_tim -> hal_board [color="#F57C00"];
    drv_adc -> hal_board [color="#F57C00"];
    drv_i2c -> hal_board [color="#F57C00"];
    drv_1wire -> hal_board [color="#F57C00"];

    hal_board -> hal_stm32 [color="#C2185B"];
    hal_stm32 -> hw_mcu [color="#303F9F"];
    hal_stm32 -> hw_uart [color="#303F9F"];
    hal_stm32 -> hw_gpio [color="#303F9F"];
    
    hw_uart -> per_motor [color="#FF8F00"];
    hw_uart -> per_lcd [color="#FF8F00"];
    hw_uart -> per_pedal [color="#FF8F00"];
    hw_uart -> per_hand [color="#FF8F00"];
    hw_gpio -> per_pump [color="#FF8F00"];
    hw_eeprom -> hal_stm32 [color="#303F9F", style=dashed];

    // ===== 强制每层宽度一致，左右对齐 =====
    // 所有层都有6个"元素"（实际节点+填充节点）
    {rank=same; app_fillL0 app_parser app_screen app_task app_fillR0 app_fillR1}
    {rank=same; mw_fillL0 mw_fillL1 mw_freertos mw_apptask mw_fillR0 mw_fillR1}
    {rank=same; drv_uart drv_gpio drv_tim drv_adc drv_i2c drv_1wire}
    {rank=same; hal_fillL0 hal_fillL1 hal_board hal_stm32 hal_fillR0 hal_fillR1}
    {rank=same; hw_fillL0 hw_mcu hw_uart hw_gpio hw_eeprom hw_fillR0}
    {rank=same; per_motor per_lcd per_pedal per_hand per_pump peri_fillR0}
}'''

dot_file = 'docs/uniform_width.dot'
with open(dot_file, 'w', encoding='utf-8') as f:
    f.write(uniform_dot)

print(f"统一宽度 DOT 文件已生成: {dot_file}")

# 生成 PNG
try:
    png_file = 'docs/uniform_width.png'
    result = subprocess.run([
        DOT_EXE, '-Tpng', '-Gdpi=200',
        '-o', png_file,
        dot_file
    ], capture_output=True, text=True)
    
    if result.returncode == 0:
        print(f"统一宽度 PNG 图片已生成: {png_file}")
    else:
        print(f"Graphviz 错误: {result.stderr}")
        
except Exception as e:
    print(f"生成 PNG 时出错: {e}")

# 生成 SVG
try:
    svg_file = 'docs/uniform_width.svg'
    result = subprocess.run([
        DOT_EXE, '-Tsvg',
        '-o', svg_file,
        dot_file
    ], capture_output=True, text=True)
    
    if result.returncode == 0:
        print(f"统一宽度 SVG 矢量图已生成: {svg_file}")
        
except Exception as e:
    print(f"生成 SVG 时出错: {e}")

print("\n=== 统一宽度生成完成 ===")
print("特点: 每层外围边框宽度一致，左右对齐")
print("方法: 使用不可见填充节点统一每层宽度")
