...
3. `MX_UART9_Init()` 仅在 `RFID_USE_DUAL_UART_MODE=1` 时执行；当前值为1，UART9会初始化，逻辑A RFID固定使用UART3，逻辑B RFID固定使用UART9。
...
A 通道入口是 `HandlescanA_Fun_SSC()`，B 通道入口是 `HandlescanB_Fun_SSC()`。A/B 在业务层始终表示逻辑通道；物理短接脚、实体运行键、EEPROM和电机通道由 `HANDLE_PHYSICAL_AB_SWAP_ENABLE` 映射。RFID另按硬件版本处理：当前双串口版固定逻辑A=UART3、逻辑B=UART9；切回R200-K8版时才由 `RFID_R200_AB_SWAP_ENABLE` 单独决定逻辑通道选择哪个固定物理通道。
...
R200-K8硬件保持固定电平含义：高电平选择物理A，低电平选择物理B。当前 `RFID_R200_AB_SWAP_ENABLE=0U`，但双串口模式不读取该配置；切回单串口模式后，逻辑A输出高电平选择物理A，逻辑B输出低电平选择物理B，需要交叉接线时可改为 `1U`。
...
业务页命令 `0x05` 只读前 30 字节有效数据；`0x07` 无论 AreaCode、长度和外控状态如何，都固定返回 EEPROM 失败 ACK `[0x07, 0x05]`，其中原因 `0x05` 表示协议不支持。Page7 和 Page10 仍是保留页，不借导航页命令开放。

导航批量读写不是一次访问整片 EEPROM。主控收到范围命令后锁定当前通道对应的 I2C2/I2C3，每个 10ms 外控任务周期累计时间，达到 30ms 后只处理一页；批量中途切换逻辑通道不会跨到另一颗 EEPROM。批量读逐页上传 30 字节，未初始化、页和错误或底层读取失败的页面返回失败实际页号后继续下一页，最终由上位机显示“部分页失败”。批量写逐页回成功 ACK，任何一页写失败仍立即停止，避免继续修改后续页面。最后一页结果与 `[批量功能码, 起始页, 结束页]` 完成 ACK 分开发送，且到期心跳会避开已发送批量帧的同一周期。批量启动失败统一使用失败对象 `0xFF`，避免批量写功能码 `0x0C` 与实际 Page12 混淆。

上位机只有在帧头、长度、CRC 和帧尾全部通过时才把心跳写入实时状态、压力曲线、泵速曲线和导出日志。Web Serial 的可恢复 `Framing error` 只写通信日志，不覆盖顶部物理连接徽标；串口真实关闭和心跳超时仍按原状态机提示。

上位机EEPROM区域在批量读写期间显示持续旋转的工作指示，并列出“已返回页/总页数、最近页面、失败页数和实时耗时”。不再使用百分比填充条，因为串口回包可能在浏览器同一处理批次内快速结算，中间宽度状态无法稳定绘制，视觉上容易从0瞬间跳满。页摘要仍只由通过顺序校验的真实成功或失败回包更新；重复、延迟和乱序帧不会改变摘要。页面全部返回但主控最终ACK尚未到达时显示“全部页面已返回，正在确认完成”。完成、部分失败、超时或断线后，旋转指示切换为对应终态图标并保留结果，点击清空EEPROM区域才隐藏。结果表仍在批量结束时统一绘制，避免逐页重绘造成界面卡顿。

外部通信上位机保持单个 HTML 和单个 Web Serial 运行实例，但界面按任务拆为四个工作台：实时监控集中显示设备状态、压力和趋势；设备控制集中显示授权、参数、通道、方向、泵和手柄动作；EEPROM维护集中显示页选择、批量活动状态、原始数据和布局解释；通信诊断集中显示日志、手动解析和配置。通信诊断不再常驻显示与日志重复的自动协议解析面板，实时 TX/RX 只写入通信日志；用户粘贴 HEX 或主动点击某条日志时，详细字段才在手动解析卡片内按需展开，清空本区后重新收起。顶部栏采用品牌、链路/控制状态、连接/急停/清空和五类导出的单行四区布局，串口状态、外控权限、当前通道、运行状态、连接按钮和长按急停固定可见。工作台切换只改变现有 DOM 面板的可见性，不重新加载页面，因此串口连接、外控保活、日志、趋势缓存和 EEPROM 批量状态都保持不变。实时监控采用“左侧状态与运行趋势、右侧压力传感器”的全高分栏；设备控制把六项运行摘要压为单行并把余量交给趋势图；EEPROM右侧的数据查看区按屏高弹性伸展。1366×768及以上桌面窗口不产生整页滚动，设备控制命令区也不需要内部上下滚动；较窄窗口自动降级为纵向布局。
...
上位机业务色在三套背景主题中保持一致：A通道使用蓝色、B通道使用紫色、速度曲线使用青色、电流曲线使用琥珀色、正常/成功使用绿色、警告使用琥珀色、故障和急停使用红色。A/B设备、刀具和压力卡使用完整圆角细描边，在线和当前工作状态继续由右上状态点及整卡背景共同表达，不使用左侧半括号式粗竖线。状态卡片、日志、按钮和Canvas曲线读取同一组主题变量；颜色只作为快速识别辅助，文字、图标、数值和状态点仍同时显示，避免仅靠颜色表达安全状态。
...
flowchart TD
    ...
    P -->|0x09/0x0C批量| B["登记起止页、模板和总线快照"]
    B --> T["ExternalComm_ServiceNavBatch()\n每30ms处理一页"]
    C --> R["原AT24CS32单页驱动"]
    T --> R
    R --> S["逐页数据或ACK\n最终返回整批完成ACK"]
```
...
| 断点 | 正常值 | 异常说明 |
| --- | --- | --- |
| `ExternalComm_DispatchFrame()` | 0x07固定拒绝；0x09/0x0C进入批量登记 | 帧头、长度、CRC 或功能码不对 |
| `ExternalComm_MapAreaToPageIndex()` | 0x05业务区域码映射到预期页 | Page7/10不在开放映射中 |
...
| `ExternalComm_ServiceNavBatch()` | 每次任务只推进一页 | 当前页、总线快照或批量操作类型异常 |
...
| 请求/应答统计 | `1` | `User/Hardware/include/board_profile.h:RFID_LINK_STATS_ENABLE` | 1时分别累计A/B实际读取命令、有效应答、丢失应答和异常帧，并追加心跳A6扩展；0时不分配计数状态且恢复原心跳格式 |
| RFID 硬件模式 | `1` | `User/Hardware/include/board_profile.h:RFID_USE_DUAL_UART_MODE` | 当前逻辑A固定使用UART3、逻辑B固定使用UART9，不经过R200-K8选通 |
| R200-K8通道交换 | `0` | `User/Hardware/include/board_profile.h:RFID_R200_AB_SWAP_ENABLE` | 只在切回单串口模式时生效；0表示逻辑A选物理A、逻辑B选物理B，高低电平物理定义不变 |
...
外部通信持有控制权时，`FootControlTask()`仍消费脚踏上线/掉线消息并刷新 `ControlSignalMessage.jt_enable_flag`、主屏脚踏图标和外控心跳字段，但收到连接状态后立即退出，不执行踏板行程、电机、泵或切通道逻辑。这样拔出脚踏后约1秒会正常显示黑色离线图标，同时外控 owner 和输出状态保持不变。
...
| `0x05` | 业务EEPROM单页读取 | Area01~08映射Page2/3/4/5/6/8/9/11；Page7/10不暴露；只上传30字节有效数据 |
...
心跳每100ms上传一次，信息区按在线/运行状态动态增减。`RFID_LINK_STATS_ENABLE=1U`时最大128字节，关闭统计时恢复原88字节上限；两种配置都低于协议信息区134字节上限。顺序和条件如下：
...
| 12 | 可选RFID链路统计扩展 | `A6 01 02`，之后固定为A、B两个17字节统计块；仅统计开关为1时存在 |
...
RFID统计块按大端编码：`Channel(1) + Request(4) + Valid(4) + Lost(4) + Invalid(4)`。`Request`只在主控实际发送EPC读取命令时增加；有效协议回包增加`Valid`；发送下一条命令时上一条仍未获得有效回包则增加`Lost`；UART收到数据但未解析出有效EPC帧则增加`Invalid`。运行、切通道、清刀具或外部停止导致的业务主动取消不计为丢包。上位机显示的丢包率固定为`Lost / (Valid + Lost)`，该指标反映主控读取请求与有效应答，不等同于射频空口PHY层误码率。计数在主控复位或重新上电后清零。
...
| 手柄物理 A/B 交换 | `User\Hardware\include\board_profile.h` | `HANDLE_PHYSICAL_AB_SWAP_ENABLE=1U` | 当前交换A/B插拔、运行键、EEPROM、电机通道和开口定位；固定RFID串口与逻辑显示不得交换 |
...
| `05` | `01..08` | 可忽略 | 业务页只读，映射Page2/3/4/5/6/8/9/11 |
...
`02/03/04`运行类操作要求对应owner和门禁；`05/08/09/0B`只读操作不要求owner，`0A/0C`导航写入必须已经取得外控owner，`07`业务写入无条件拒绝。业务页读回前30字节，页尾2字节校验由驱动验证；Page7和Page10不在业务Area映射内。导航单页读回使用 `Area=FF、Info=请求页码`。12..117同时落在两种单页导航编码区间，代码优先按实际页解释；批量命令只接受无歧义的实际页12..128。批量读写还要求手柄与A/B泵均停止，运行状态改变会在下一页前中止本批。
...
当前 `RFID_USE_DUAL_UART_MODE=1`：逻辑A RFID固定使用UART3，逻辑B RFID固定使用UART9，不经过R200-K8选通。切回单串口模式后，A/B两套模块才经R200-K8共用UART3；高电平固定选择物理A，低电平固定选择物理B，`RFID_R200_AB_SWAP_ENABLE=0` 表示逻辑A选物理A、逻辑B选物理B。无论使用哪种硬件模式，RFID结果都写回请求所属的逻辑A/B缓存。地区 `RFID_REGION_SELECT=RFID_REGION_US=0x02`，发射功率 `RFID_TX_POWER_SELECT=RFID_TX_POWER_10_DBM_X100=1000`，即10dBm。每个模块的初始化顺序是发射功率、等待50ms、地区、等待50ms、跳频、等待50ms：
...
| RFID逻辑A路/单串口共享口 | USART3，PD8/PD9 | 115200，8N2，TX/RX | DMA1 Stream1 Channel4 | 当前固定供逻辑A使用；切回单串口模式后配合PB15选通A/B模块 |
...
| RFID双串口B路 | UART9，PD15/PD14 | 115200，8N2，TX/RX | DMA2 Stream7 Channel0 | 当前已初始化并固定供逻辑B使用；单串口模式不初始化 |
...
| --- | ---: | --- | --- | --- |
| 手柄物理A/B交换 | `1U` | `User/Hardware/include/board_profile.h:HANDLE_PHYSICAL_AB_SWAP_ENABLE` | C | 当前映射短接、运行键、EEPROM和电机到对侧物理资源；固定RFID串口、逻辑消息、UI和报警不得交换 |
| R200-K8 RFID A/B交换 | `0U` | `User/Hardware/include/board_profile.h:RFID_R200_AB_SWAP_ENABLE` | C | 仅切回单串口硬件时生效；高电平物理A、低电平物理B定义保持不变 |
...
| 单/双串口 | `1U` | `board_profile.h:RFID_USE_DUAL_UART_MODE` | C | 当前逻辑A固定UART3、逻辑B固定UART9；设为0后才由R200-K8选通两套模块共用UART3 |
...
| 开双RFID串口 | `board_profile.h:RFID_USE_DUAL_UART_MODE` | 当前1启用UART9，固定UART3=逻辑A、UART9=逻辑B；改0后恢复UART3+R200-K8 | 1 |
...
| 外控心跳信息区 | 统计开启128U、关闭88U | `external_comm_task.c:EXTERNAL_COMM_HEARTBEAT_INFO_MAX_LEN` | 协议/容量边界；开启时追加37字节A/B RFID统计，仍受134字节协议信息区上限约束 |
...
| RFID请求/应答统计 | `RFID_LINK_STATS_ENABLE=1U` | `User/Hardware/include/board_profile.h` | 可改/C/D类；1时心跳追加A6统计，0时完全恢复原计数内存和心跳格式；切换后必须同步上位机协议版本并重新构建 |
