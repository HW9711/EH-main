...
编制基线：`R1` 分支，`41bcb10cddd01ebf537dc0429a90232353279b50`
...
| 最近一次已记录构建 | 127个C文件、1个汇编文件，0 error、0 warning | 127个C文件、1个汇编文件，0 error、0 warning | 本轮三项可靠性修复已按EIDE实际参数完成全量构建 |
...
| Page4 阈值只蜂鸣 | `WORK_ALARM_SPEED_THRESHOLD` 不应当阻塞调速 | 调速到阈值附近机器被误停 | `Pubinterface_CheckSpeedThresholdAlarm()` |
...
| 快速/普通次数 | 10 / 3 | `RFID_FAST_ATTEMPTS`、`RFID_NORMAL_ATTEMPTS` | 每次实际发送请求后递减剩余次数；有效EPC提前结束，未成功时按10次或3次自然耗尽 |
...
`s_request_attempts_left`在每次实际发送RFID读取命令后递减。请求收到有效EPC时提前结束；没有有效标签时，快速请求最多发送10次、普通请求最多发送3次，然后退出当前请求，避免无线程永久轮询。
...
| A/B压力板 | PE4/PE6软串口 | CS1237 v2、设备码0x0B/0x0D/0x0E、11字节扩展和CRC16/MODBUS | 设备码决定泵类型；压力来源固定A=SIM1、B=SIM2 | 三类设备码、正负压力、掉线和CRC错误分别抓帧 |
...
| 外部通信上位机 | UART2 | 外框Length/CRC、心跳A5、统计A6版本2、EEPROM 0x05~0x0C及0x0B软件版本 | 8字节申请只校验长度；业务写0x07固定拒绝；导航写要求owner | 用第24.5节全套用例回归，并保存主控0x0B版本载荷和代表帧 |
...
| 文档同步 | 本手册维护记录、正文、配置表、协议表和测试表 | `git diff --cached -- docs/软件说明手册.md` | 与软件修改同一提交并可追溯 |
