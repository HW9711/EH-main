...
| 在线监测与掉线判断 | 200ms，连续缺失10次 | `HANDLESCAN_RFID_MONITOR_PERIOD_MS`、`HANDLESCAN_RFID_MISS_MAX` | 约2秒无有效标签后清除刀具、蜂鸣一次并累计一次确认掉线；持续离线不重复累计或蜂鸣 |
...
`MonitorCompleted`来自handlescan在线监测结算：标签presence序号得到确认或到下一监测周期仍未确认，都完成一次监测。`ConfirmedDropout`只在此前在线的RFID刀具连续缺失达到`HANDLESCAN_RFID_MISS_MAX`、真正清刀具并蜂鸣时增加一次；持续离线不重复增加。上位机请求丢包率为`Lost / (Valid + Lost)`，主控确认掉线率为`ConfirmedDropout / MonitorCompleted`，两者含义不同且都在主控复位或重新上电后清零。
...
| 外控停输出超时 | `external_comm_task.c` | 2000ms | 静默2s停止电机和泵输出，但保留A/B泵`speed_work`设定值 |
