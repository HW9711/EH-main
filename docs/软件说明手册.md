...
编制基线：`R1` 分支，`187da75b6eb249c82813262068363ecb4d8c0411` 后续工作区
...
| 最近一次已记录构建 | 128个C文件、1个汇编文件，0 error、0 warning | 128个C文件、1个汇编文件，0 error、0 warning | 当前工作区增加固定方向门禁和RFID电流单位换算，构建清单未增加源文件 |
...
| 日期 | 软件基线 | 手册变更 | 验证结论 |
| --- | --- | --- | --- |
| 2026-07-24 | `R1` / `187da75` 后续工作区 | TMBA/TMBB/EMBA/EMBB/JMB/MXYTM锁定EEPROM默认方向，当前方向高亮、其它方向显示灰色禁用态，本地与外控均不能换向；MXYTP界面固定往复而UART1固定下发正转；RFID标签电流阈值按0.1A读取并转换为主控/驱动0.01A单位，标签100对应10.00A；手柄物理A/B交换值仍以目标线束和`board_profile.h`为准 | EIDE/AC5全量构建128个C、1个汇编，0 error、0 warning；RO 110240B、RW 130760B、ROM 110724B；最终HEX SHA256为 `5807FD84A6E9753EFA8E4CE925D79F3A6044767008A328C3BCAED4489ED73001` |
...
| 手柄型号 | 界面方向 | 输入门禁 | 驱动实际方向 |
| --- | --- | --- | --- |
| TMBA、TMBB、EMBA、EMBB、JMB、MXYTM | EEPROM Page4默认方向高亮，其它两个方向显示灰色禁用图标 | 触屏、实体键/HMI和外控换向全部拒绝 | 按EEPROM默认方向 |
| MXYTP | 往复高亮，正转和反转显示灰色禁用图标 | 触屏、实体键/HMI和外控换向全部拒绝 | UART1始终下发正转，由机械结构实现往复 |
...
驱动原始 Err、主控统一报警码和屏幕图片号必须分开理解。`MOTOR_ALARM_DETAIL_ENABLE` 位于 `motoruartdata.h`，默认 `0U`；它只切换驱动报警图片，不改变停机、泵保护、蜂鸣、脚踏过载锁存和外控报警码。

| 驱动 Err | 驱动含义 | 主控统一报警码 | 宏 `0U` 生产图片 | 宏 `1U` 内部调试图片 |
| ---: | --- | ---: | --- | --- |
...
| 85 | UID错误，请联系售后 | 统一报警码7 |
| 86 | HALL值错误，请联系售后 | 统一报警码9；宏关闭时驱动 `Err=11/12` 共用 |
| 87 | 电机过载，请查看刀具是否卡死 | 统一报警码4/5；驱动 `Err=5` 始终使用，宏关闭时 `Err=2` 共用 |
| 88 | 外部控制中 | 屏幕资源已存在，当前主控不作为报警弹窗调用；外控状态仍使用既有在线状态图标 |
| 89 | 泵压力异常，请检查 | 统一报警码15的限时压力提示 |
| 90 | 手柄校验异常，请联系厂家 | 统一报警码10/12/14 |
| 91~99 | 驱动细分调试报警 | 仅 `MOTOR_ALARM_DETAIL_ENABLE=1U` 时按上表使用，91~99不写入 `WorkMessage.alarm_value` |
...
压力扩展固定为 `RawCs1237(int32 LE) + WeightX10(uint32 LE) + ThresholdG(uint16 LE) + Seq`，共11字节。刀具20字节块依次为：通道、来源（0 EEPROM/1 EPC）、工具类型、直径、长度BE16、角度、默认速度BE16、最小速度BE16、最大速度BE16、默认流量BE16、方向、传动比BE32。A5版本1的传动比高16位增速、低16位减速且单位为x10；A5版本2保持相同20字节块和高低位定义，但单位升级为x100。上位机必须同时接受两个版本：版本1按 `/10`、版本2按 `/100` 显示两位小数，历史原始值 `0/1` 均显示等效 `1.00` 倍。外框和普通字段是大端，压力扩展是小端，上位机必须逐字段解析。
...
| 3 | `WORK_ALARM_FOOT_SELECTED` | 当前已选择脚踏模式却使用手柄键启动 | 通常为限时提示 | UI 显示82号脚踏已选中 | 切换控制方式 |
| 4 | `WORK_ALARM_MOTOR_OVERLOAD` | 旧兼容过载入口；当前驱动 Err 不再使用该码 | 视来源 | UI 显示87号过载图 | 按调用模块规则清除 |
| 5 | `WORK_ALARM_MOTOR_OVERLOAD_ALT` | 驱动 Err 2/5 | 是 | 蜂鸣和外控仍上传5；宏关闭显示87，宏开启时Err2显示92、Err5仍显示87 | 非脚踏来源按原规则清除；脚踏来源必须驱动恢复且当前脚踏已松开 |
| 6 | `WORK_ALARM_FOOT_VALUE_ERROR` | 脚踏定标值错误 | 是 | UI 脚踏值错误 | 重新定标或修复脚踏数据 |
| 7 | `WORK_ALARM_UID_ERROR` | UID 错误 | 是 | UI UID 错误 | 修复 UID/识别数据 |
| 8 | `WORK_ALARM_MOTOR_COMM_ERROR` | 驱动 Err 3/4 或通信异常 | 是 | 蜂鸣和外控上传8；Err3仅在调试宏开启时显示96，Err4及普通通信异常不借用84号图 | 驱动恢复 |
| 9 | `WORK_ALARM_HALL_ERROR` | Hall 值异常或驱动 Err 11/12 | 是 | 非驱动来源显示86；驱动来源宏关闭显示86，宏开启分别显示93/94 | 修复 Hall/驱动反馈 |
| 10 | A EEPROM 校验失败 | `Handlescan_RaiseAlarm(A, ...)` | 是或限时提示 | A 手柄型号错误 | A 拔出、重试通过或降级为另一通道报警 |
| 11 | Page4 阈值蜂鸣或 `WORK_ALARM_MOTOR_DRIVER_BOARD` | Page4阈值，或驱动 Err 1/6/7/8/9/10/13/14/15 | 阈值不是，驱动板是 | 驱动Err14始终显示84；调试宏开启时6/8/9/10/15显示97/99/91/95/98；1/7/13不分配图片 | 阈值恢复或驱动恢复 |
...
报警数值不是全局唯一枚举：同一个数字在提示路径和真实故障路径中可能有不同来源。驱动排查必须同时记录“产生模块 + 原始驱动 Err + 主控统一报警码 + 屏幕图片号”，不能把91~99图片号当成外控报警码。`WorkMessage.alarm_value` 也只是单一全局槽，不是报警优先队列；后写入者可能覆盖先前值。
...
| 电机驱动 Err | UART1 回包 Err 非 0 | 是 | 脚踏来源由25ms任务锁存停机；恢复帧再执行全停 | A泵立即直发0，A/B `run_flag=false` | 按原始Err和调试宏选择图片；未分配图片的Err不借用84 | 普通故障至少3秒；脚踏过载等待Err恢复且当前松脚 | 始终上传主控统一报警码，不上传91~99图片号 |
...
| 驱动细分报警图片 | `0U` | `User/Application/include/motoruartdata.h:MOTOR_ALARM_DETAIL_ENABLE` | B/调试 | `0U`使用84/86/87公用图片；`1U`启用91~99内部调试图片；两态的停机、蜂鸣、脚踏锁存和外控报警码必须一致 |
...
| 驱动细分报警图 | `MOTOR_ALARM_DETAIL_ENABLE=0U` | `User/Application/include/motoruartdata.h` | 调试/B类；量产默认关闭，内部联调开启后只改变91~99图片选择，不能改变统一报警码和安全动作 |
