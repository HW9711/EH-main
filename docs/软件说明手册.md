...
浏览器阅读可选用 `docs/打开并自动更新软件说明手册.cmd` 启动本地预览。当前电脑安装了文件加密软件，不要用直接双击 HTML 的显示结果判断文档是否损坏。`docs/软件说明手册.md` 仍是唯一文档真值；HTML 只作本地临时预览，后续软件提交不再要求同步生成或提交 HTML。

当前正式发布基线与未提交工作区必须分开理解：

| 项目 | `cf94e12` Git基线 | 当前工作区 | 使用结论 |
| --- | --- | --- | --- |
...
| 最近一次已记录构建 | 128个C文件、1个汇编文件，全量构建通过 | 128个C文件、1个汇编文件，0 error、0 warning | 当前工作区完成注水泵图标、量程、默认值和共用步进适配；RO 122936B、RW 135744B、ROM 123500B，HEX SHA256为`F00938D6E7CFA817F47B5898BA0916C0B00CB963F1E4FD3D99D7328D95E0AF50` |
...
| 日期 | 软件基线 | 手册变更 | 验证结论 |
| --- | --- | --- | --- |
| 2026-08-19 | `R1` / `cf94e12` 后续工作区 | 注水泵设备码继续保持`0x07→INJECTWATER`，识别在线标题图改用灌注泵亮态资源213，停用态仍保留210；识别边沿固定装载默认流量30，业务量程、Page4默认流量有效范围和十格档位环统一扩展到300；注水泵与灌注泵A/B调速均改用`PUMP_WATER_FLOW_STEP=30U`共用宏。注水泵排空、手柄冷却联动、压力保护、`/1.6`驱动换算和其它泵类型保持不变 | EIDE/AC5实际构建路径全量重建通过：128个C文件、1个汇编文件，0 error、0 warning；RO 122936B、RW 135744B、ROM 123500B；HEX SHA256为`F00938D6E7CFA817F47B5898BA0916C0B00CB963F1E4FD3D99D7328D95E0AF50`。静态检查确认A/B均使用共用步进宏、注水上限与Page4校验均为300、设备码映射未变；仍需实机验证接入显示30/图213、30递增至300及压力负载下实际流量 |
| 2026-08-10 | `R1` / `6257225` 后续工作区 | 新增EEPROM手柄DHYTM：Page2编码`6B 10`，不使用RFID，按磨削能力装载2.00倍增速、10000～120000rpm、默认120000rpm、默认流量30、电流阈值500（5.00A）及1000/5000调速步进；屏幕固定显示反转并锁定换向，A通道复用109/110、B通道复用139/140，驱动实际固定下发正转并使用无刷无霍尔模式。公共接头物理插入只在第一轮RFID请求时蜂鸣，第二轮内部重试静音；基座已显示在线但仍处于首包或重试等待时拔出，继续执行500ms确认并发布一次离线事件。保留快速插拔导航代次与重新认证门禁，同时移除依赖异步在线位的宽泛拔出兜底，避免同一次拔出重复蜂鸣 | EIDE/AC5全量构建通过：128个C文件、1个汇编文件，0 error、0 warning；RO 116316B、RW 135520B、ROM 116876B；HEX SHA256为`D433EDAB1EC385AF8A95B3FC1A6B3FAB446EF463140EE96AC89682C6FA65B070`。DHYTM配置已由配套EEPROM工具和外部通信上位机完成独立验证，本次主控提交不包含仓库外文件 |
| 2026-08-06 | `R1` / `425a952` 后续工作区 | 修正快速插拔期间导航目标仍被旧在线状态放行的问题：扫描层首次检测到已上线手柄拔出候选时立即递增导航代次，使单页和批量在途请求失效；原约500ms拔出去抖保持不变，500ms内插回必须重新执行约50ms插入消抖、约200ms认证前等待和EEPROM认证。外控导航开始、逐页处理和最终确认均检查导航就绪态与扫描代次；重新认证期间的新请求返回原因`0x06`，已经开始的请求被插拔打断时返回原因`0x08`；导航V2帧格式、命令号、心跳和协议版本不变 | EIDE/AC5全量构建通过：128个C文件、1个汇编文件，0 error、0 warning；RO 116184B、RW 135520B、ROM 116744B；HEX SHA256为`DC4641CA60D5C06D3D7E7AD92A8E0162014238326000D2B132060C2054FE4D11`。同步更新UART2协议DOCX的快速插拔时序、原因码、重试要求和联调用例；DOCX完成OpenXML结构、内容标记和表格模板校验；本机缺少LibreOffice，Word只读导出又被文件保护插件阻塞，未生成页面图像 |
| 2026-08-03 | `R1` / `a8c1902` 后续工作区 | 导航EEPROM `0x08/0x09/0x0A/0x0C`增加V2显式目标格式：下行携带逻辑A/B与16位请求号，逐页数据、写成功ACK、最终ACK和失败ACK均回显关联字段；旧导航帧仅在恰好一个通道在线时兼容，双通道在线返回目标不明确。单页在操作前后复核在线态、身份代数和16字节SN；批量开始锁定逻辑通道、物理总线、身份代数和SN，逐页只做无阻塞RAM校验，整批结束统一复核SN；中途拔柄、换柄或映射变化返回原因`0x08`且不再发送成功完成ACK | EIDE/AC5构建通过，0 error、0 warning；RO 116028B、RW 135504B、ROM 116588B，HEX SHA256为`9F5B152334744D7B38344EBC243B64AC05308E09A00687D960CA32CDC73CC8A7`；同步更新UART2协议DOCX、导航V2字段、时序、失败语义和联调用例 |
| 2026-07-30 | `R1` / `b08ff26` 后续工作区 | A/B两个RFID物理模块初始化时均先发送厂家稳定性参数`BB 00 F0 00 04 03 06 02 80 7F 7E`并等待50ms，再设置10dBm功率、中国920频段和跳频；手柄校验失败报警清除后增加跨任务关窗请求，使90号弹窗关闭消息在无手柄UI队列复位后重新发送；持续蜂鸣、另一通道坏手柄继承及其它全局报警门禁保持不变 | R200-K8和双串口配置均完成EIDE/AC5全量构建，128个C文件、1个汇编文件，0 error、0 warning；双串口RO 114408B、RW 135616B、ROM 114968B；恢复正式R200-K8配置后RO 114200B、RW 135472B、ROM 114760B，HEX SHA256为`9E7BCE3109C9922C449A29C81714EA448258C468B66BA5D65AC6A16CCA9F4544` |
...
| 页面字段 | 编码方式 | 当前影响 |
| --- | --- | --- |
| Page2 手柄/工具代码 | 2 字节 | 映射 TMBB、TMBA、EMBA、EMBB、PXBA、PXBB、DHYTM等基座或手柄类型；DHYTM固定使用`6B 10` |
...
| Page4 默认流量 | 2 字节大端，范围 1~300 | 注水泵默认流量；0 或越界时按程序默认值30处理 |
...
外控只能读取业务页，不能写入识别、刀具、运行参数和出厂数据。导航 Page12~Page128 允许单页和批量读写，但写入前必须已经取得外部控制权。导航 V2 请求不再依赖易变的 `WorkMessage.channel_work`，而是在 InforArea 中显式携带逻辑 A/B 和16位请求号；主控再通过 `HANDLE_PHYSICAL_AB_SWAP_ENABLE` 把逻辑目标换算为物理 I2C2/I2C3。通道在线显示只表示事件层尚未确认离线，不等于导航EEPROM已经就绪；快速插回重新认证期间必须由扫描状态门禁拒绝访问。旧导航帧仅在恰好一个通道在线时兼容，A/B双在线时返回原因 `0x07` 目标不明确，不能猜当前选中通道：

| 导航目标逻辑通道 | 开启交换 `1U` | 关闭交换 `0U`（`425a952`正式基线） |
| ------ | --------- | ------------------------ |
| A | I2C3 | I2C2 |
| B | I2C2 | I2C3 |
| 非A/B或目标离线 | 读写失败 | 读写失败 |
...
导航 V2 公共前缀固定为 `[02, Channel, RequestId_H, RequestId_L]`，其中 Channel 为 `01=A`、`02=B`。主控接受请求时除确认目标通道在线，还要求该通道扫描状态已经完成本轮EEPROM识别并稳定处于导航就绪态；随后锁定逻辑通道、物理总线、扫描导航代次、身份代数，并从同一总线读取16字节 EEPROM SN。单页操作完成后再次核对导航就绪态、两类代次、物理映射和SN，任一不一致就丢弃数据或不确认写成功。

导航批量读写不是一次访问整片 EEPROM。主控登记范围时锁定上述目标快照，每个10ms外控任务周期累计时间，达到30ms后只处理一页；逐页访问前后比较导航就绪态、扫描导航代次、在线态、物理映射和身份代数，不再为每一页追加阻塞式SN读取，因此切换当前选中通道不会改变本批目标，也不会把A/B数据串配。首次检测到已上线手柄拔出候选时，扫描层立即递增导航代次，不等待约500ms拔出确认；已经开始的导航任务在下一次前后复核时以原因`0x08`中止。若500ms内重新插回，扫描层重新执行约50ms插入消抖、约200ms认证前等待和EEPROM认证；这段时间在线显示可能仍保留，但新导航请求以原因`0x06`拒绝，外部设备应等待后使用新的RequestId重试。

最后一页逐页结果发出后，主控再统一读取一次SN。只有导航就绪态、扫描导航代次、在线态、身份代数、物理总线和16字节SN仍与开始快照一致，才连续发送3次V2最终成功ACK `[02, 原功能码, Channel, RequestId_H, RequestId_L, 起始页, 结束页]`。该ACK是独立UART2数据帧，不在心跳内；中途拔柄、换柄或扫描代次变化则发送 `DD/06`，原因 `0x08`，且不再发送成功完成ACK。外部设备必须按 Channel、RequestId 和页号关联逐页结果，并等待最终ACK后才把整批数据提交给业务层；只收齐页面但没有最终ACK，仍不能判成功。117页按30ms节拍的理论逐页时间约3.51秒，实际还要叠加I2C访问、最终SN复核和UART发送时间，超时应按持续无有效进展计算。

上位机只有在帧头、长度、CRC 和帧尾全部通过时才把心跳写入实时状态、压力曲线、泵速曲线和导出日志。Web Serial 的可恢复 `Framing error` 只写通信日志，不覆盖顶部物理连接徽标；串口真实关闭和心跳超时仍按原状态机提示。

上位机EEPROM区域在批量读写期间显示持续旋转的工作指示，并列出“已返回页/总页数、最近页面、失败页数和实时耗时”。不再使用百分比填充条，因为串口回包可能在浏览器同一处理批次内快速结算，中间宽度状态无法稳定绘制，视觉上容易从0瞬间跳满。页摘要只由目标范围内尚未结算的真实成功或失败回包更新；乱序页面可以正常进入结果集合，重复、延迟或范围外页面不会重复计数。导航V2即使全部逐页结果到齐，也必须继续等待与Channel、RequestId、范围一致的最终ACK；若最终ACK先到但仍缺读页，则只补读缺失区间，补齐后再结束。完成、失败、补读耗尽、超时或断线后，旋转指示切换为对应终态图标并保留结果，点击清空EEPROM区域才隐藏。结果表仍在批量结束时统一绘制，避免逐页重绘造成界面卡顿。
...
| FunCode | 导航V2请求字段 | 运行规则 |
| ------- | ------------ | -------- |
| `0x06` | `AreaCode=起始页`，`InforArea[0]=结束页` | 业务批量读保持旧格式；每30ms原始读取并返回一页 |
| `0x08` | `Area=实际页12~128`；`InforArea=[02,Channel,ReqH,ReqL]` | 显式目标单页读；返回数据帧回显原功能码和请求号 |
| `0x09` | `Area=起始页`；`InforArea=[02,Channel,ReqH,ReqL,结束页]` | 显式目标批量读；逐页数据和最终ACK都属于同一请求 |
| `0x0A` | `Area=实际页`；`InforArea=[02,Channel,ReqH,ReqL,数据30字节]` | 显式目标单页写；要求外控权 |
| `0x0C` | `Area=起始页`；`InforArea=[02,Channel,ReqH,ReqL,结束页,模板30字节]` | 显式目标批量写；要求外控权，每页写同一模板 |

旧导航格式仍按原长度兼容，但只允许恰好一个通道在线；新接入设备必须使用V2，不能通过先切换当前通道再发送无通道请求来推断数据归属。

```mermaid
flowchart TD
    H["外部设备导航V2命令：显式Channel和RequestId"] --> P["ExternalComm_DispatchFrame()"]
    P -->|0x07业务写| X["固定拒绝，不访问EEPROM"]
    P -->|0x08/0x0A| C["检查导航就绪；锁定通道、物理总线、扫描代次、身份代数和SN"]
    P -->|扫描未就绪| BUSY["DD/06原因06；等待重认证后以新RequestId重试"]
    P -->|0x09/0x0C| B["登记范围、请求号、扫描代次、目标快照和模板"]
    B --> T["每30ms处理一页；逐页检查导航就绪和RAM代次"]
    C --> R["原AT24CS32单页驱动"]
    T --> R
    R --> S["逐页数据或ACK；回显Channel和RequestId"]
    S --> F["整批结束统一复核SN"]
    F -->|一致| OK["最终成功ACK连续3次"]
    F -->|离线、换柄或扫描代次变化| NG["DD/06原因08；不发送成功完成ACK"]
```
...
| `ExternalComm_CaptureNavTarget()` | 按V2显式通道检查导航就绪，捕获物理总线、扫描导航代次、身份代数和16字节SN | 快速插回仍在重新认证时返回原因06；目标码无效、离线或稳定目标SN读取失败时按对应原因拒绝 |
| `ExternalComm_StartNavBatch()` | 起止页12~128，请求号已登记；写入时外控有效且全部输出停止 | 长度、范围、外控权、运行状态或忙状态不满足；返回对应失败原因 |
| `ExternalComm_IsNavTargetRamStable()` | 每页访问前后导航就绪态、扫描导航代次、在线态、物理总线和身份代数保持不变 | 拔柄、换柄、快速插拔或映射改变时立即中止并返回原因08 |
| `ExternalComm_IsNavTargetIdentityStable()` | 单页完成或整批结束时16字节SN与开始快照一致 | SN无法读取或发生身份变化时不得发送成功完成ACK |
| `ExternalComm_ServiceEepromBatch()` | 每30ms最多推进一页，任一页I2C失败立即中止 | 当前页、目标快照、运行门禁或批量操作类型异常 |
...
| 外控写导航 Page12~Page128 | 导航V2显式指定的A/B通道 EEPROM 导航数据 | 是 | 单页或批量写成功后立即落盘；批量写每页使用同一模板，中断时已写页不回滚 |
...
快速插拔时，事件层的 `WorkMessage.Channel_Aonline/Channel_Bonline` 可以为了500ms防抖暂时保持在线，但扫描层已经进入非ONLINE状态并递增导航代次。外部通信必须以 `Handlescan_IsNavigationReady()` 和 `Handlescan_GetNavigationGeneration()` 为导航访问依据，不能只看心跳或当前通道在线位。
...
| EEPROM手柄DHYTM（Page2 `6B 10`） | 固定反转高亮，正转和往复显示灰色禁用图标；A通道使用109/110，B通道使用139/140 | 触屏、实体键/HMI和外控换向全部拒绝；该手柄没有实体运行按钮且不读取RFID | UART1固定下发正转，按无刷无霍尔模式运行；界面方向不回写 |
...
| 配置或状态 | 当前值 | 入口 | 影响 |
| --- | --- | --- | --- |
| 地区频段 | `RFID_REGION_CHINA_920`（`0x01`） | `User/Application/Beep/sscRFID.c:RFID_REGION_SELECT` | 生成中国920.125～924.875MHz区域命令和校验和；交付前仍需核对销售地区法规 |
...
| 稳定性参数与跳频配置 | `F0 03 06 02 80 7F`、`AD FF` | `User/Application/Beep/sscRFID.c:rfid_stability_command/hop_ch` | A/B物理模块均先发送厂家F0稳定性参数并等待50ms，再设置功率、地区和跳频；当前没有F2/F3扫描、A9动态列表或F1读回流程 |
...
三类速度必须分开观察：普通注水和手柄联动受 `PUMP_INJECTWATER_SPEED_MAX=300U` 限制；屏幕定时排空单独使用 `PUMP_TIMING_DRAINAGE_SPEED=100U`；压力闭环允许并准备下发的业务速度保存在 `speed_output`。注水泵和灌注泵的A/B加减流量共同读取 `PUMP_WATER_FLOW_STEP=30U`，改宏后两类泵保持相同步进。任何 UI 或外控显示问题都应同时记录这些来源，物理泵是否转动还要看步进板或水路实测。
...
注水泵上限是 `PUMP_INJECTWATER_SPEED_MAX = 300`。外控或屏幕写更大的注水速度，最终也会被泵任务限幅。注水泵和灌注泵共用 `PUMP_WATER_FLOW_STEP = 30`，A/B屏幕或HMI每次加减都按该宏变化；需要调整步进时只改此宏。

压力板设备码仍按 `0x07` 识别为 `INJECTWATER`，类型码没有改。类型或在线状态形成识别边沿时，注水泵先把 `speed_work` 装载为30并显示在线标题图213；停用态标题仍使用210。后续手柄通道成为当前工作通道时，合法的Page4默认流量仍可在1～300范围内写入注水泵，排空、手柄冷却联动、压力保护和驱动 `/1.6` 换算保持原逻辑。
...
| `INJECTWATER` 普通注水 | 0 | `PUMP_INJECTWATER_SPEED_MAX=300` | `speed / 1.6`，浮点结果截成整数 | 300档→187 |
| `INJECTWATER` 屏幕定时排空 | 0 | 独立固定100 | 同样 `speed / 1.6`，先经过压力保护 | 100档→62 |
| `POURWATER` 灌注 | 0 | 300 | `speed / 1.51`，浮点结果截成整数 | 300档→198 |

排空旧高转速公式已经删除；当前不存在 `(speed*0.02+2.1)*speed`。排空只是把注水业务目标临时设为100，再走与普通注水相同的压力保护和 `/1.6` 换算。普通注水结束排空后最多允许300。
...
| `0x08/0x0A` | 导航页单页读/写 | V2信息区以`02、Channel、RequestId_H、RequestId_L`开头，显式锁定A/B目标；0x0A再携带30字节且要求外控权 |
| `0x09` | 导航页范围批量读取 | V2信息区携带目标通道、请求号和结束页；每30ms最多读一页，任一页失败、拔柄或换柄立即中止，不再报告部分成功 |
| `0x0B` | 读取主板软件版本 | 要求Area/Info为FF且信息区为空，上传状态字节加Page1原始32字节 |
| `0x0C` | 导航页范围批量写入 | V2信息区携带目标通道、请求号、结束页和30字节模板；要求外控权，任一页失败立即中止，已写页不回滚 |
...
| 注水/灌注泵调速步进 | `pump.h` | `PUMP_WATER_FLOW_STEP` | A/B屏幕与HMI加减流量，并确认两类泵保持相同步进 |
...
| 注水/灌注泵步进 | `User\Application\include\pump.h` | `PUMP_WATER_FLOW_STEP` | A/B两类水泵加减流量共用步进，当前为30 | 分别接入注水泵和灌注泵，确认每次加减都变化30且上限不越界 |
...
| EEPROM 维护 | 读业务页、读写导航页；业务页写固定拒绝 | 业务页按当前通道；导航V2按请求中显式A/B目标锁定 AT24CS32 | 业务读立即返回原始30字节；导航写成功后立即落盘，中断返回独立失败帧 |
...
| 注水泵上限与水泵步进 | `PUMP_INJECTWATER_SPEED_MAX` 影响注水泵速度钳位；`PUMP_WATER_FLOW_STEP` 同时影响注水泵和灌注泵A/B加减流量，不改变三类泵类型码 |
...
| 泵上限与步进 | 注水泵上限写 `300`；注水泵和灌注泵步进写共用宏当前值`30`，不要把上限泛化到所有泵类型 |
...
| 普通注水 `INJECTWATER` | 300 | `speed / 1.6`，整数截断 | 0 |
...
| `08` | 实际页12..128 | `02 Channel ReqH ReqL` | 导航V2单页读；回包显式携带通道、请求号、页号和30字节原始数据 |
| `0A` | 实际页12..128 | `02 Channel ReqH ReqL Data30` | 导航V2单页写；要求外控owner，成功ACK回显通道、请求号和页号 |
| `09` | 起始实际页 | `02 Channel ReqH ReqL EndPage` | 导航V2批量读；锁定目标身份，每30ms最多读一页，任一异常立即中止 |
| `0B` | `FF` | 空 | 读主板软件版本，上传状态字节+Page1原始32字节 |
| `0C` | 起始实际页 | `02 Channel ReqH ReqL EndPage Data30` | 导航V2模板批量写；要求外控owner，每页写同一模板，任一异常立即中止 |
| `BB` | 任意 | 任意 | 退出外控、停输出并释放owner |
| `FA` | 任意 | 恰8字节 | 权限占位；只回权限ACK，不建立权限、认证或owner状态 |

`02/03/04`运行类操作要求对应owner和门禁；`05/08/09/0B`只读操作不要求owner，`0A/0C`导航写入必须已经取得外控owner，`07`业务写入无条件拒绝。外控维护读取业务页和导航页时直接上传原始前30字节，不执行页和校验；Page3读取因此会原样保留 `[8..16]` 的兼容倍率布局，A5刀具扩展才按版本上传已经装载的运行倍率。全 `FF` 空页和其它原始内容均按指定范围返回，只有 I2C 访问失败才产生失败页。Page7/Page10使用独立Area09/0A读取。导航V2只接受实际页12..128，公共前缀显式给出Channel与RequestId；旧导航格式仅在恰好一个通道在线时兼容，两通道同时在线固定拒绝并返回原因07。切换当前选中通道不会改变已锁定任务；批量期间拔掉或更换目标手柄、I2C访问失败、映射变化或运动输出启动，都会立即中止并发送独立失败帧。
...
| `05/06` | EEPROM操作成功/失败 | 导航V2单页写成功为`[02,原功能码,Channel,ReqH,ReqL,Page]`；批量最终成功为`[02,原功能码,Channel,ReqH,ReqL,Start,End]`并连发3次；失败见下文 |
| `AA/AB` | 外控申请成功/失败 | `AA`成功无载荷；owner忙时`AB`使用2字节失败载荷 |
| `BB` | 0x01外控申请的8字节注册码长度校验失败 | 无载荷 |
| `FE/FF` | 0xFA权限占位成功/失败 | 均无载荷，且不代表已取得外控owner |

旧通用失败ACK由 `ExternalComm_SendFailAck()` 发送2字节载荷 `[失败对象, REASON]`。导航V2失败改用独立载荷 `[02,原功能码,Channel,ReqH,ReqL,TargetPage,Reason]`，可明确指出哪一通道、哪一请求和哪一页中断；该帧不是心跳。原因码为：`01`长度错、`02`Area错、`03`无通道、`04`设备访问失败、`05`不支持、`06`忙、`07`旧格式下目标通道歧义、`08`目标手柄离线或身份变化。批量写中断时已经写入的页面不会回滚，且主控不再发送最终成功ACK。注册码和权限码长度检查的无载荷ACK不使用这套失败对象；退出成功应答使用 `INFO=03、INFO_AREA=[BB]`。
...
`RFID_USE_DUAL_UART_MODE=0U`时，A/B两套RFID经R200-K8共用UART3且不初始化UART9；高电平固定选择物理A，低电平固定选择物理B，`RFID_R200_AB_SWAP_ENABLE`决定逻辑A/B是否交换选通。设为双串口模式后，UART3固定服务逻辑A、UART9固定服务逻辑B并绕过R200-K8。两项宏必须按目标硬件选择，手册不规定固定交付值。无论使用哪种硬件模式，活动事务开始后物理通道保持到明确成功、超时或取消，结果按A/B独立票据写回所属逻辑通道。当前地区为`RFID_REGION_CHINA_920=0x01`，发射功率为10dBm。每个物理模块均先发送厂家F0稳定性参数并等待50ms，再依次设置功率、中国920频段和固定跳频；双串口分别通过UART3/UART9执行，R200-K8依次选通A/B后通过共享UART3执行：

| 命令 | 完整字节 |
| --- | --- |
| 设置稳定性参数 | `BB 00 F0 00 04 03 06 02 80 7F 7E` |
| 设置发射功率 | `BB 00 B6 00 02 03 E8 A3 7E` |
| 设置中国920.125～924.875MHz频段 | `BB 00 07 00 01 01 09 7E` |
| 开启跳频 | `BB 00 AD 00 01 FF AD 7E` |
| 无掩码读EPC | `BB 00 22 00 00 22 7E` |

地区命令校验字节按`0x08+REGION`派生；功率协议值以dBm的100倍表示，高低字节和累加校验均由`RFID_TX_POWER_SELECT`派生。F0帧是当前固定的模块启动稳定性参数，A/B两侧均在其它射频配置前执行；当前没有F2/F3环境扫描、A9动态信道表或F1读回验证。
...
当前手柄拔出确认仍需要连续约500ms；插入确认约50ms。该500ms用于稳定事件层在线状态、UI和报警，不再延迟导航数据保护：首次检测到已上线手柄拔出候选时就立即递增扫描导航代次，使在途导航读取失效；500ms内插回也必须重新等待约50ms并完成约200ms认证前等待及EEPROM认证，认证完成前导航请求返回原因`0x06`。公共接头基座在第一轮RFID读取开始时只蜂鸣一次，第二轮内部容错重试不再重复蜂鸣；若基座已经显示在线但扫描状态仍为`WAIT_RFID_TOOL`或`RETRY_WAIT`，拔出同样进入500ms确认并发布一次离线事件。该判断只依赖本通道RFID等待标志和扫描状态，不读取异步`WorkMessage.Channel_*online`，因此离线事件发布后不会重复进入拔出流程。拔出保护的第一动作仍是停输出，报警是后续用户提示，所以即使UI队列或蜂鸣队列忙，停机动作也不依赖弹窗是否成功显示。
...
校验报警清除时，扫描任务先清统一报警和持续蜂鸣。最后一把手柄拔出后，`UIDP_ForceNoHandleDisplay()`会重置UI队列，因此关闭90号图片的旧消息可能被清掉；当前使用通道级`verify_alarm_close_pending`请求跨任务保留关窗意图，并在无手柄UI复位完成后重新发送`UI_AIARM_ID=false`。如果另一通道仍是坏手柄，或系统仍有其它全局报警，则不关闭弹窗。
...
| `ChannelrecognizeMessageA/B.default_injection_flow` | Page4 | 跟随注水泵 | 1~300 或回退 30 | 默认注水 |
...
| `s_a_context.verify_alarm_close_pending/s_b_context.verify_alarm_close_pending` | 校验报警恢复或拔出清除入口 | 插拔事件任务 | true/false | true表示90号关窗请求需跨越无手柄UI队列复位；消费前仍需确认没有其它全局报警 |
...
| 配置项 | `PUMP_INJECTWATER_SPEED_MAX`、`PUMP_WATER_FLOW_STEP`；当前值见第27.2节 |
| 影响范围 | 前者控制注水泵速度钳位；后者控制注水泵与灌注泵A/B加减流量步进 |
| 不影响 | 抽吸泵和灌注泵类型本身 |
...
| EXT-07 | 导航V2批量读 | A/B均在线 | 指定A、请求号1235读取Page12~20，处理中切换当前选中通道到B | 目标仍锁定A，逐页回包均回显A和1235 | 全部页面后最终ACK连续3次，范围一致 | `ExternalComm_StartNavBatch()` |
| EXT-08 | 导航V2批量写 | 已取得外控owner且设备空闲 | 指定B、请求号1237写Page12~20同值模板 | 任一页失败立即终止，已写页不回滚 | 逐页成功ACK及最终ACK均回显B和1237 | `ExternalComm_FinishEepromBatch()` |
...
| EXT-11 | 最终ACK重复容错 | 导航V2批量完成 | 接收3个相同完成ACK并模拟丢失其中1~2个 | 完成状态只结算一次；仅收齐逐页数据不得判成功 | Channel、RequestId和范围必须全匹配 | 主控 `ExternalComm_FinishEepromBatch()` 与外部设备去重 |
| EXT-12 | 批量中途运行门禁 | 批量正在进行 | 启动手柄或A/B泵 | 下一页处理前中止批量 | 返回失败且不继续访问后续页 | `ExternalComm_ServiceEepromBatch()` |
| EXT-13 | 批量中途拔柄 | 指定A启动Page12~128读取 | 收到若干页后拔掉A，B保持在线 | 扫描导航代次在首次拔出候选时立即变化，不等待500ms离线确认 | V2失败帧回显A、原请求号、目标页和原因08；无最终成功ACK | `ExternalComm_IsNavTargetRamStable()` |
| EXT-14 | 批量中途换柄 | 指定A启动批量且记住原SN | 中途快速拔A并插入另一只A手柄 | 扫描导航代次先使旧任务失效；新手柄重新识别后身份代数或SN也必须更新 | 旧任务返回原因08，新请求在重认证完成前返回原因06 | `ExternalComm_IsNavTargetIdentityStable()` |
| EXT-15 | 旧格式双通道门禁 | A/B均在线 | 发送不带V2前缀的0x08/09请求 | 不读取任一EEPROM | 返回原因07目标歧义 | `ExternalComm_CaptureNavTarget()` |
| EXT-16 | 简易协议登录和启停 | 开关1U、当前手柄在线 | 发送01登录、03启动、04停止、02退出 | owner和运行请求按顺序进入/退出 | 6字节指令本身无ACK，原心跳反映状态 | `ExtSimple_HandleFrame()` |
| EXT-17 | 简易协议半包/粘包 | UART2可分段注入 | 拆分6字节帧并与原协议帧交错发送 | 半帧保留；按最早帧头依次处理 | 原协议载荷内的AA BB CC不被误执行 | `ExtSimple_Probe()`、`ExternalComm_ProcessRxFifoFrame()` |
| EXT-18 | 简易协议关闭 | 宏设为0U并重新全量构建 | 发送全部01～0C帧 | owner、运行和泵状态均不变化 | UART2仅识别原协议 | `EXTERNAL_COMM_SIMPLE_PROTOCOL_ENABLE` |
| EXT-19 | 清除异常提醒安全边界 | 存在真实安全报警 | 发送0C后再次发送03 | 蜂鸣/弹窗关闭但`alarm_flag`保持 | 电机仍不能启动 | `EXT_SIMPLE_FUN_CLEAR_REMINDER` |
| EXT-20 | 快速插回导航门禁 | A通道已在线，外部设备能正常读取导航页 | 拔出A并在500ms内重新插入，心跳仍可能显示A在线，立即发单页及批量读 | 扫描状态重新进入插入消抖和认证等待，导航代次已变化 | 重认证期间返回原因06；完成认证后以新RequestId重试成功且数据属于当前A手柄 | `Handlescan_ProcessDisconnect()`、`ExternalComm_CaptureNavTarget()` |
...
| 普通注水上限 | `300U` | `pump.h:PUMP_INJECTWATER_SPEED_MAX` | A | 屏幕、脚踏、手柄联动、外控均测；不改变屏幕定时排空的独立100档 |
| 注水/灌注泵调速步进 | `30U` | `pump.h:PUMP_WATER_FLOW_STEP` | A | A/B两路、屏幕和HMI均共用；修改后分别验证两类泵加减流量 |
...
| 屏幕定时排空速度 | `100U` | `pump.h:PUMP_TIMING_DRAINAGE_SPEED` | A | 只用于屏幕排空；验证普通注水仍按300上限而不继承排空状态 |
...
| 屏幕/HMI调节步长 | 抽吸1、注水30、灌注30 | `pump_control.c` 与 `pump.h:PUMP_WATER_FLOW_STEP` | A | 注水和灌注共用宏；验证A/B上、下边界和数值显示 |
| 脚踏档位表 | 抽吸 `3×档`；注水0/10/20/30/50/70；灌注0/50/100/150/200/300 | `pump_control.c` | A | 修改后覆盖每档与压力闭环 |
| 默认注水流量范围 | 1~300，回退30 | `handlescan.c:HANDLESCAN_INITIAL_FLOW_*` 与 `Pubinterface.c:HANDLE_INJECTION_PUMP_FLOW_*` | A | 两组重复定义必须同步；有效Page4会覆盖识别边沿装载的30 |
...
| RFID配置 | 当前值 | 入口 | 分类 | 注意 |
| --- | --- | --- | --- | --- |
| 地区 | 中国920 `RFID_REGION_CHINA_920` | `sscRFID.c:RFID_REGION_SELECT` | C/D | 当前使用920.125～924.875MHz；交付前仍需按销售地区法规确认 |
| 稳定性参数/跳频 | 固定`F0 03 06 02 80 7F`、`AD FF` | `sscRFID.c:rfid_stability_command/hop_ch` | C/D | A/B均先发送F0并等待50ms，再配置功率、地区和跳频；当前没有F2/F3/A9/F1试验流程 |
...
| 改普通注水上限 | `pump.h:PUMP_INJECTWATER_SPEED_MAX`，同步两组默认流量范围 | 屏幕、脚踏、手柄、外控、排空仍固定100 | 300 |
| 改注水/灌注泵步进 | `pump.h:PUMP_WATER_FLOW_STEP` | A/B两路分别加减，注水和灌注步进一致，达到上限后不越界 | 30 |
...
| 改RFID地区 | `sscRFID.c:RFID_REGION_SELECT` | 法规频段、A/B初始化、EPC读取 | 中国920 |
...
| RFID地区选择 | `RFID_REGION_SELECT=RFID_REGION_CHINA_920` | `User/Application/Beep/sscRFID.c` | 可改/C/D类；当前使用中国920.125～924.875MHz，可选美国、欧洲、中国840或韩国，交付前核对法规 |
| RFID地区校验 | `RFID_REGION_COMMAND_CHECKSUM=(0x08U+RFID_REGION_SELECT)` | 同上 | 派生常量；随地区选择自动生成，不要再手工写死校验字节 |
| RFID稳定性参数 | `rfid_stability_command={BB 00 F0 00 04 03 06 02 80 7F 7E}` | `User/Application/Beep/sscRFID.c` | 固定初始化命令；A/B物理模块均最先发送并等待50ms，降低噪声导致的偶发识别掉线 |
| RFID跳频命令 | `hop_ch={BB 00 AD 00 01 FF AD 7E}` | `User/Application/Beep/sscRFID.c` | 固定初始化命令；A/B物理模块各在稳定性参数、功率和地区设置后发送一次 |
...
- 屏幕定时排空为25ms周期、10000ms、速度100；普通注水上限为300，注水/灌注屏幕与HMI共用30步进。
...
4. HTML仅作为可选本地预览，不再要求运行 `node tools/software_manual/build_manual.js`，也不再把 `docs/软件说明手册.html` 纳入软件提交闸门。
5. 将代码、工程配置和 `docs/软件说明手册.md` 放入同一个提交；Markdown未同步暂存时不得提交软件变更。
...
| 文档同步 | 本手册维护记录、正文、配置表、协议表和测试表 | `git diff --cached -- docs/软件说明手册.md` | Markdown与软件修改同一提交且可追溯；HTML为可选本地预览，不要求同步或提交 |
