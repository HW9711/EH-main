...
| 最近一次已记录构建 | 128个C文件、1个汇编文件，0 error、0 warning | 128个C文件、1个汇编文件，0 error、0 warning | 快速插拔时立即作废导航访问并重新认证，单页和批量绑定扫描导航代次；RO 116184B、RW 135520B、ROM 116744B，HEX SHA256为`DC4641CA60D5C06D3D7E7AD92A8E0162014238326000D2B132060C2054FE4D11` |
...
当前手柄拔出确认仍需要连续约500ms；插入确认约50ms。该500ms用于稳定事件层在线状态、UI和报警，不再延迟导航数据保护：首次检测到已上线手柄拔出候选时就立即递增扫描导航代次，使在途导航读取失效；500ms内插回也必须重新等待约50ms并完成约200ms认证前等待及EEPROM认证，认证完成前导航请求返回原因`0x06`。拔出保护的第一动作仍是停输出，报警是后续用户提示，所以即使UI队列或蜂鸣队列忙，停机动作也不依赖弹窗是否成功显示。
