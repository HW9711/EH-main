...
| 最近一次已记录构建 | 128个C文件、1个汇编文件，0 error、0 warning | 128个C文件、1个汇编文件，0 error、0 warning | 当前工作区增加导航EEPROM V2显式通道与请求号、手柄身份代数、批量目标锁定及最终SN复核；正式R200-K8配置RO 116028B、RW 135504B、ROM 116588B，HEX SHA256为`9F5B152334744D7B38344EBC243B64AC05308E09A00687D960CA32CDC73CC8A7` |
...
外控只能读取业务页，不能写入识别、刀具、运行参数和出厂数据。导航 Page12~Page128 允许单页和批量读写，但写入前必须已经取得外部控制权。导航 V2 请求不再依赖易变的 `WorkMessage.channel_work`，而是在 InforArea 中显式携带逻辑 A/B 和16位请求号；主控再通过 `HANDLE_PHYSICAL_AB_SWAP_ENABLE` 把逻辑目标换算为物理 I2C2/I2C3。旧导航帧仅在恰好一个通道在线时兼容，A/B双在线时返回原因 `0x07`“目标不明确”，不能猜当前选中通道：

| 导航目标逻辑通道 | 开启交换 `1U` | 关闭交换 `0U`（`a8c1902`正式基线） |
| ------ | --------- | ------------------------ |
| A | I2C3 | I2C2 |
| B | I2C2 | I2C3 |
| 非A/B或目标离线 | 读写失败 | 读写失败 |
...
导航 V2 公共前缀固定为 `[02, Channel, RequestId_H, RequestId_L]`，其中 Channel 为 `01=A`、`02=B`。主控接受请求时确认目标通道在线，锁定逻辑通道、物理总线、该通道身份代数，并从同一总线读取16字节 EEPROM SN；单页操作完成后再次核对RAM状态和SN，身份不一致就丢弃数据或不确认写成功。

导航批量读写不是一次访问整片 EEPROM。主控登记范围时锁定上述目标快照，每个10ms外控任务周期累计时间，达到30ms后只处理一页；逐页访问前后只比较在线态、物理映射和身份代数，不再为每一页追加阻塞式SN读取，因此切换“当前选中通道”不会改变本批目标，也不会把A/B数据串配。导航批量读或写任一页发生I2C失败、目标手柄离线/换柄、映射变化、运动输出启动，或者批量写期间外控权失效，都会立即中止整批并发送独立失败ACK，不再继续后续页面；已经物理写入的页面不会自动回滚。

最后一页逐页结果发出后，主控再统一读取一次SN。只有在线态、身份代数、物理总线和16字节SN仍与开始快照一致，才连续发送3次V2最终成功ACK `[02, 原功能码, Channel, RequestId_H, RequestId_L, 起始页, 结束页]`。该ACK是独立UART2数据帧，不在心跳内；中途拔柄或换柄则发送 `DD/06`，原因 `0x08`，且不再发送成功完成ACK。外部设备必须按 Channel、RequestId 和页号关联逐页结果，并等待最终ACK后才把整批数据提交给业务层；只收齐页面但没有最终ACK，仍不能判成功。117页按30ms节拍的理论逐页时间约3.51秒，实际还要叠加I2C访问、最终SN复核和UART发送时间，超时应按“持续无有效进展”计算。
...
flowchart TD
    ...
    P -->|0x08/0x0A| C["捕获目标通道、物理总线、身份代数和SN"]
    P -->|0x09/0x0C| B["登记范围、请求号、目标快照和模板"]
    B --> T["每30ms处理一页；逐页只检查RAM身份"]
    C --> R["原AT24CS32单页驱动"]
    T --> R
    R --> S["逐页数据或ACK；回显Channel和RequestId"]
    S --> F["整批结束统一复核SN"]
    F -->|一致| OK["最终成功ACK连续3次"]
    F -->|离线或换柄| NG["DD/06原因08；不发送成功完成ACK"]
```
...
| `ExternalComm_CaptureNavTarget()` | 按V2显式通道捕获物理总线、身份代数和16字节SN | 通道离线、目标码无效或SN读取失败时不能启动导航操作 |
| `ExternalComm_StartNavBatch()` | 起止页12~128，请求号已登记；写入时外控有效且全部输出停止 | 长度、范围、外控权、运行状态或忙状态不满足；返回对应失败原因 |
| `ExternalComm_IsNavTargetRamStable()` | 每页访问前后在线态、物理总线和身份代数保持不变 | 拔柄、换柄或映射改变时立即中止并返回原因08 |
...
| EXT-13 | 批量中途拔柄 | 指定A启动Page12~128读取 | 收到若干页后拔掉A，B保持在线 | 下一页前识别目标离线并中止 | V2失败帧回显A、原请求号、目标页和原因08；无最终成功ACK | `ExternalComm_IsNavTargetRamStable()` |
| EXT-14 | 批量中途换柄 | 指定A启动批量且记住原SN | 中途快速拔A并插入另一只A手柄 | 身份代数或最终SN复核失败 | 返回原因08，旧任务结果不得提交业务层 | `ExternalComm_IsNavTargetIdentityStable()` |
