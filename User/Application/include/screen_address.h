...
#define UIDP_LCD_VP_PUMP_B_BUTTON 0x1422U
/* B泵加号VP：当前代码用于刷新B泵上调按钮，后续地址校正时统一从此宏调整。 */
#define UIDP_LCD_VP_PUMP_B_PLUS 0x1423U
/* A泵减号VP：当前代码用于刷新A泵下调按钮，后续地址校正时统一从此宏调整。 */
#define UIDP_LCD_VP_PUMP_A_MINUS 0x1424U
/* B泵减号VP：当前代码用于刷新B泵下调按钮，后续地址校正时统一从此宏调整。 */
#define UIDP_LCD_VP_PUMP_B_MINUS 0x1425U
/* A泵单位VP：当前代码用于刷新A泵流量单位，后续地址校正时统一从此宏调整。 */
#define UIDP_LCD_VP_PUMP_A_UNIT 0x1426U
/* B泵单位VP：当前代码用于刷新B泵流量单位，后续地址校正时统一从此宏调整。 */
#define UIDP_LCD_VP_PUMP_B_UNIT 0x1427U
/* 报警提示VP：当前代码用于刷新报警提示图片，后续地址校正时统一从此宏调整。 */
#define UIDP_LCD_VP_ALARM_TIP 0x1428U
/* A泵启停旧VP：当前代码仍写入A泵启停按钮的历史地址，暂不在本轮修正。 */
#define UIDP_LCD_VP_PUMP_A_BUTTON 0x1606U
/* 转速数值VP：用于写入主电机转速数值。 */
