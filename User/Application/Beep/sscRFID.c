...
static unsigned char  pa_gain20[]={0XBB, 0X00, 0XB6, 0X00, 0X02, 0X0A, 0X28, 0Xea, 0X7E};//发射功率
static unsigned char  pa_gain10[]={0XBB, 0X00, 0XB6, 0X00, 0X02, 0X03, 0Xe8, 0Xa3, 0X7E};//发射功率
static unsigned char  pa_gain3[]={0XBB, 0X00, 0XB6, 0X00, 0X02, 0X01, 0X2C, 0XE5, 0X7E}; //发射功率
...
// 添加RfidHandle函数的声明
void RfidHandle(uint8_t * uartx_rf_buff, uint8_t interface,bool enable_rfid);
...
static void SplitType_AutoModeGetData_Task(void)         //Check_Connect(void)
{
    ...
    if(rfid_start_flag)
    {
        ...
        else
        {
            ...
            if (rlen < 7)   //不够一个数据包大小
           return ;
            RfidHandle(dat,rfid_channel,true);
        }
    }
    else
    {
        //不发送
        RfidHandle(dat,rfid_channel,false);
    }
