...
/*
 * 函数功能：A 泵收到新的启动请求沿时清除压力锁止。
 * 输入参数：request_active 表示本周期 A 泵是否存在运行或排空请求。
 * 返回参数：无。
 */
static void PUMPA_ClearPressureHoldOnNewRequest(uint8_t request_active)
{
	static uint8_t last_request_active = 0U; /* 保存上一周期 A 泵是否有输出请求，用于判断这次是不是新的控制源启动沿。 */

	if ((request_active != 0U) && (last_request_active == 0U))
	{
		pumpMessageA.pressure_hold_flag = false; /* 只有新的启动沿才解除 A 泵压力锁止，压力自然下降不能自动恢复运行。 */
		pumpMessageA.pressure_recover_ms = 0U;  /* 保留字段同步清零，避免后续调试误读为仍在恢复计时。 */
	}

	last_request_active = request_active; /* 记录本周期请求状态，连续踩住脚踏或触控保活不会被当成新的启动沿。 */
}

static uint16_t PUMPA_ApplyPressureClosedLoopRaw(uint16_t pump_speed, uint8_t *pressure_force_stop)
{
...
/*
 * 函数功能：按压力阈值和锁止策略处理 A 泵本周期输出速度。
 * 输入参数：pump_speed 为上层准备输出的 A 泵速度，pressure_force_stop 用于返回本周期是否被压力锁止压停。
 * 返回参数：A 泵压力闭环后的实际输出速度，0 表示本周期必须停泵。
 */
static uint16_t PUMPA_ApplyPressureClosedLoop(uint16_t pump_speed, uint8_t *pressure_force_stop)
{
	const pumpMessage_t *pressure_source = PUMPA_GetPressureSource(); /* 固定读取 A 泵压力源，当前由 soft_uart.c 的 PE4 压力帧更新。 */
	...
	if (pumpMessageA.pressure_hold_flag == true)
	{
		pumpMessageA.pressure_recover_ms = 0U; /* 当前策略不再按压力恢复时间自动解除锁止，保持字段清零便于调试观察。 */
		Pubinterface_ServicePumpPressureHold(CHANNEL_A); /* 抽吸/注水/灌注泵锁止保持期间继续停 A 泵，只有注水冷却手柄时才联动停手柄。 */
		if (pressure_force_stop != NULL)
		{
	...
	{
		pumpMessageA.pressure_hold_flag = true; /* 新触发停泵阈值后进入压力锁止，不允许压力刚掉下去就恢复。 */
		pumpMessageA.pressure_recover_ms = 0U; /* 当前策略不做恢复消抖，保留字段清零表示锁止刚建立。 */
		if (pressure_force_stop != NULL)
		{
			*pressure_force_stop = 1U; /* 返回压力停泵标志，调用方保留原有停泵分支结构。 */
		}
		Pubinterface_HandlePumpPressureBlocked(CHANNEL_A); /* A 泵三种类型触发阈值都先停泵并蜂鸣，注水冷却手柄时再联动停手柄。 */
		return 0U; /* 达到停泵阈值的首个周期立即输出 0。 */
	}

	return PUMPA_ApplyPressureClosedLoopRaw(pump_speed, pressure_force_stop); /* 未触发保持时沿用原线性限速算法。 */
}

static void PUMPA_PauseByPressureLimit(void)
{
	/* 实际压力锁止已在 PUMPA_ApplyPressureClosedLoop() 内完成，这里只保留原调用结构。 */
	/*
...
/*
 * 函数功能：按 pumpMessageA 状态计算 A 泵本周期输出，处理注水排空、压力闭环和 UART 下发。
 * 输入参数：无。
 * 返回参数：无。
 */
void PUMPAehaviors()
...
static uint8_t huici=0;//互斥
		...
		uint8_t request_active = 0U; /* 记录本周期 A 泵是否有运行请求，用于判断压力锁止是否遇到新的启动沿。 */
		 ...
		 */
		request_active = (pumpMessageA.run_flag || pumpMessageA.timingDrainage_flag) ? 1U : 0U; /* run_flag 或排空任一有效都代表 A 泵有输出请求。 */
		PUMPA_ClearPressureHoldOnNewRequest(request_active); /* 只有从无请求到有请求时才释放压力锁止，避免压力卸掉后自动恢复。 */
		timing_drainage_active = pumpMessageA.timingDrainage_flag ? 1U : 0U; /* 先锁存排空状态，避免后续压力分支修改标志后本周期判断前后不一致。 */
		if(pumpMessageA.run_flag || pumpMessageA.timingDrainage_flag)
		{
			pump_type=pumpMessageA.type;
			pump_speed = (timing_drainage_active != 0U) ? PUMP_TIMING_DRAINAGE_SPEED : pumpMessageA.speed_work; /* 排空模式固定使用 70 档业务速度，保证 A/B 泵排空转速一致。 */
		}
		switch(pump_type)
		{
			case DRAWWATER: //抽
		...
		if(timing_drainage_active != 0U)
		{
           ...
           else if(pumpMessageA.timingDrainage_times >= PUMP_TIMING_DRAINAGE_TICKS)
		   {
			uart_data=0;
		   ...
		   {
			pumpMessageA.timingDrainage_times++; /* A 泵排空未到 10 秒时只累计时间，UART 速度沿用注水 70 的正常换算结果。 */
		   }
		}
