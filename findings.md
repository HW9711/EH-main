...
## 2026-08-10 PX分体式手柄方案核实

- `pc_tool_new/profiles/eeprom_profiles.csv` 当前 PXBA/PXBB 的 Page4 通用参数均为默认转速30000、最小转速2500、最大转速70000；PXBA/PXBB手动模式恢复该通用Page4参数，无法为刨刀往复单独提供500～6000范围。
- 当前RFID速度字节由同一换算常量按1000rpm解释；新规则只要求“最小转速字段0x00表示500rpm”，设计时必须只特殊处理EPC byte7，不能把最大/默认速度字段的0值一起改成500。
- CSV中PXBA电流阈值为120、PXBB为120，单位沿Page4现有定义为0.01A；后续需核实RFID识别失败或普通刀具场景中该值是否被清零或覆盖。
- RFID成功解析前会先`memset`清空通道识别缓存，随后只用EPC byte11重建三方向电流阈值；基座Page4电流值未被暂存恢复，因此标签值为0或缺少有效标签时无法自然回退到手柄EEPROM保护值。
- 当前无刷运行类型选择函数只接收`hand_model`，PXBA/PXBB没有按RFID刀具的单向/电气往复/机械往复能力区分有无霍尔；新设计必须以“RFID基座+原始刀具型号+实际电机运动”判定，不能仅看屏幕方向。
- PXBB当前同时出现在手控可用型号表和实体按键切换模型表，现状确实会显示手控入口并允许进入HANDLEWORK，与“PXBB无手柄按钮”的硬件事实冲突。
- 当前CSV实际生成的PXBA/PXBB Page3 `[17..29]`均为13个`00`，Page4 `[23..29]`也为空；但PXBA样例的Page5存在原始30字节覆盖，因此优先使用Page3剩余13字节承载带版本标记的手动刨刀参数，避免占用已有Page5数据。
- PX手动模式当前调用同一Page4恢复函数，刨刀和磨头共享默认/最小/最大速度；刨刀入口还保留30000兜底。新协议应让Page4继续承载手动磨头/基座通用参数，Page3扩展只承载手动刨刀默认/最小/最大速度和频率。
- PXBB禁用手控需要同时收口三层：`Pubinterface_IsHandleControlReservedModel()`界面/模式门禁、`HandleRunKey_IsSupportedModel()`实体键白名单，以及通道装载/脚踏离线回落时对历史`HANDLEWORK`的归一化。
- 分体手柄电流保护建议采用“非零RFID标签阈值 > 非零手柄Page4阈值 > 0（驱动默认）”的优先级，并独立保存基座阈值；手动切换、无标签上线和RFID确认掉线都恢复Page4值。
- 驱动`run_type`应依据转换后的实际电机方向决定：实际OSCDIR使用无刷有霍尔`0x02`，实际ZZDIR/FZDIR使用无刷无霍尔`0x01`；MXYTP显示往复但实际方向已转换为ZZDIR，因此必须归无霍尔。

## 2026-08-10 RFID标签安全校验新增约束

- 用户已确认PXBA/PXBB手动刨刀默认6000rpm；RFID电流0回退EEPROM；有/无霍尔规则只作用PXBA/PXBB。
- 当前EPC 12字节已经全部占用：型号、直径、长度、角度、2字节倍率、最高/最低/默认速度、默认流量、方向和电流；增加认证字段必须压缩或重编码现有参数，不能简单追加字节。
- 参数范围和跨字段约束只能防止危险参数进入运行态，不能证明标签来自本厂；普通CRC也只能发现误码，无法阻止知道算法的人伪造。
- 带密钥MAC可以阻止不知道密钥者随意生成或修改参数，但如果MAC只覆盖12字节EPC，完整复制一张合法标签仍可通过；防整包克隆需要把标签不可改写TID/UID纳入MAC，前提是当前模块能稳定读取TID。
- R200协议族资料表明可通过标签存储区读取命令访问TID，但当前使用的模块固件必须台架确认0x39是否绑定刚选中的同一标签、响应字节序和最坏时延；在此闭环前不能宣称已具备防克隆能力。
- GPT-5.6 Sol Pro复核推荐量产使用`Profile-ID + TID + MAC64`，把所有安全参数移回主控批准表；该方案比自由参数签名更能限制密钥泄露或签发错误后的危险参数空间。
- 推荐V2 EPC布局为：byte0固定`0xA2`，byte1为非零`key_epoch`，byte2..3为大端`profile_id`，byte4..11为AES-128-CMAC最左64位；MAC输入包含固定域、产品族、格式、EPC[0..3]和完整规范化TID。
- 24位MAC明确不适合量产防伪：单次随机通过概率为`2^-24`，且攻击者可跨主机并行把控制器当验证器；限速只能延缓，不能提高认证位数。
- 手动模式必须区分“确实无标签”和“发现标签但认证失败”：前者可按已确认的EEPROM/固定安全包络运行，后者不能自动降级到手动模式，否则会形成认证旁路。
- Pro认为MXYTM规则不足，但当前项目事实已明确MXYTM是标签锁定的单向刀具，显示方向与电机方向均跟随标签；因此最终本地方案保留现有MXYTM规则，不采纳该条默认拒绝建议。

## 2026-08-10 DHYTM跨工程核实

- 当前主控内部型号编号使用1～21，Page2已用到`6B 0F`；`6B 10`在当前源码中未占用，但是否采用仍需作为协议决定确认。
- EEPROM上位机仍保留退役的`MXYTM / 7C 01`配置，其120000rpm默认/上限、10000rpm下限、流量30、2倍增速和1000/5000步进与DHYTM几乎一致，仅电流350和方向正转不同；复用`7C 01`会让历史MXYTM EEPROM被新固件识别为DHYTM。
- DHYTM适合走现有“一体式EEPROM手柄”分支：Page2识别型号，Page3只提供2.00倍增速，Page4提供反转显示方向、速度、流量和500电流阈值，Page6提供调速步进。
- 公共接头图标可只在`MapHandleToUiType()`把DHYTM映射到公共接头UI类别，不应把DHYTM加入RFID公共接头业务判断，否则会错误要求EPC刀具标签。
- 当前驱动默认对非PXY手柄下发无刷电机并使用无霍尔`run_type=0x01`；DHYTM只需在方向输出层把屏幕反转转换为电机正转。
- 外部通信主控心跳已原样上传Page2两个型号字节；外部通信上位机只需增加同一型号编码到`HAND_TYPES`名称表，无需修改心跳协议长度。
- EEPROM写入固件UART服务本身按原始页数据通用写入，不需要型号分支；只有要求板端旧“固定写入样例”直接写DHYTM时，才需要替换`Core/Src/main.c`的固定Page2～Page8数组。
- 尚需用户确认：DHYTM是否复用退役`7C 01`或使用新编码、是否存在实体手控按键、500是否明确表示5.00A，以及旧固定写入样例是否改成DHYTM。
- 用户已确认DHYTM使用全新编码；本轮统一分配当前未占用的Page2`6B 10`，不重新解释历史MXYTM EEPROM。
- 用户已确认DHYTM不读取RFID、没有实体手柄按钮，电流阈值500按Page4现有0.01A单位解释为5.00A。
- 用户要求DHYTM必须写入EEPROM上位机正式配置文件`pc_tool_new/profiles/eeprom_profiles.csv`，CSV应成为GUI、页生成和EXE的统一参数来源。
- DHYTM CSV已写入`6B 10`、增速比2.00、流量30、默认/上限120000、下限10000、方向2、电流500和1000/5000步进；生成页断言确认Page2、Page3、Page4和Page6字节与主控协议一致。
- EEPROM写入固件按任意Page2～Page8原始页通用写入，不需要增加DHYTM分支；本轮没有替换用户现有`Core/Src/main.c`固定样例。
- 外部上位机`HAND_TYPES`已增加`6B 10 -> DHYTM`，正式心跳回归可从原始Page2字节解析出DHYTM名称。
- EEPROM全量测试当前受`pc_tool_new`迁移遗留影响：部分旧用例仍硬编码已删除的`pc_tool`路径，系统临时目录受限，另有PXBA旧期望值与现CSV不一致；这些失败与DHYTM字节断言无关。
- 用户关闭两个运行实例后，已通过临时`pc_tool`包名映射从`pc_tool_new`源码完成PyInstaller单文件打包；正式`pc_tool_new/dist/EEPROMTool.exe`已覆盖并通过启动冒烟，SHA256为`259BD9A0F0B394B7752156FA40D65A50C007F95CE875924D9E53420862B1501D`。

## 2026-08-10 脚踏松开后无刷手柄偶发不停机核实

- 正常松脚没有“一次性松开队列消息”。`FootMessage_t`只保存连接状态、脚踏类型和定标值；单踏板、双段及双脚踏实时AD由UART4解析任务直接写入全局值，25ms行为任务每周期读取这些值决定启动或停止。
- `Foot_SendMessage()`只在脚踏上线、掉线和首次读取定标参数时调用，因此`FootMsgQueue`丢消息不能直接解释一次正常松脚后不停机。
- UART4掉线计数只在本周期DMA总长度小于10字节时执行；若收到大于等于10字节的噪声或坏帧而没有任何有效脚踏帧，旧的踩下高AD仍被保留，掉线计数也不增加。
- `Foot_ParseUartFrame()`一见到`FE EF`候选帧就先把`silent_ticks`清零，尚未验证命令、完整长度或CRC；运行期双段/双脚踏在线后只需10字节前缀就更新实时AD并返回，未使用脚踏板周期帧已经提供的CRC。
- 上述解析缺口可以形成完整故障链：踩下时保存高AD；松开帧丢失或损坏后旧高AD继续驱动`runflag_work=true`；噪声或伪帧又阻止掉线超时；再次踩下产生有效帧后仍能继续调速。
- 双脚踏另有确定的左右标志错误：左侧从电机段退回轻踩段时检查/清除`jtR_control_flag`，右侧对称分支检查/清除`jtL_control_flag`。若松脚AD停在低点阈值以上的中间区，本侧运行标志可能保持，直到进入完整释放分支才清除。
- `Foot_SendMessage()`先更新去重缓存、再忽略`Kernel_QueueSend()`返回值；队列满时掉线消息可能发送失败且不再重试。这是次要的掉线安全缺口，但不是正常松脚主路径。
- `WorkMessage.runflag_work=false`后，`MOTORRUNTask`每50ms直接经UART1发送零速度停止帧，不经过脚踏队列。参考GE2433无刷驱动把零速度识别为停止并进入制动，停止帧固定电机类型不构成本次无刷问题的首要原因。
- 建议最小修复：只在通过完整长度、类型和CRC校验的实时帧后更新AD并复位在线计数；连续无有效实时帧时优先清除脚踏运行请求，再按原1秒阈值更新离线UI；修正双脚踏左右标志；队列缓存只在发送成功后更新。

## 2026-08-10 当前主控提交边界
- 当前工作区位于游离 `425a952`，本地 `R1` 指向其后继 `6257225`；直接提交会产生无人引用的游离提交，不能作为正式交付。
- `6257225` 包含需要保留的导航关联与重认证门禁，但其 `work_online` 宽泛拔出兜底会在异步离线消息尚未消费时重复进入拔出去抖。
- 当前工作区采用更窄条件：仅当RFID等待已经启动且状态为 `WAIT_RFID_TOOL/RETRY_WAIT` 时补入拔出去抖；离线发布后同步清零等待标志，不依赖异步公共在线位。
- 当前源码还包含DHYTM `6B 10`、固定反向显示/实际正转、无刷无霍尔及公共接头图标映射；这些内容需要随本次正式R1提交同步写入唯一软件说明手册。
- 正式R1合并后的扫描状态机仍在首次拔出候选时递增 `navigation_generation`，因此外控在途导航立即失效；RFID等待态拔出确认后同步清除 `rfid_wait_started`，下一轮处于IDLE时不会再次进入拔出流程。
